...
def F1_score(pred, y):
    ...
    pred_cpu = pred.cpu().numpy() if isinstance(pred, torch.Tensor) else pred

# 调用 f1_score
    f1 = metrics.f1_score(y_cpu, pred_cpu)
    return f1
...
def train_MDPAT(net, optimizer, targets, e_cig, e_ctg, detail_is_acs, rates, exercises, logger, args, optimize_target):
    net.to(args.device)
    net.train()
...
def test_MDPAT(net, targets, e_cig, e_ctg, detail_is_acs, rates, exercises, logger, args, optimize_target):
    net.to(args.device)
    net.eval()
    N = int(math.ceil(len(targets) / args.batch_size))
    if 'rate' in optimize_target:
