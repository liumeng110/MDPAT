from data import MDPAT_DATA
from model import MDPAT
from run import train_MDPAT as train
from run import test_MDPAT as test
...
def train_one_dataset(train_data, val_data, test_data, logger, target, args):
    model = MDPAT(args.seq_len, args.num_exercises, args.embedding_dim, args.device, args.max_position, args.dropout if args.dropout > 0 else 0.2)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
    logger.info(model)
    print(model)
    D = MDPAT_DATA(args.seq_len)
    ...
    if args.do_test:
        ...
        print('test performance: AUC: {0}, F1: {1}'.format(test_performance['auc'], test_performance['f1']))
        logger.info('AUC: {:.6f}, F1: {:.6f}'.format(test_performance['auc'], test_performance['f1']))
        print('test performance: AUC: {0}, F1: {1}'.format(test_performance['auc'], test_performance['f1']))
